...
const EditRecipe = () => {
    ...
    const handleCalculateCalories = async () => {
        ...
        try {
            ...
            setError('Failed to calculate calories');
        } finally {
    ...
    return (
        <div className="max-w-4xl mx-auto animate-slide-up pb-20">
            <button 
                ...
                className="flex items-center gap-2 text-slate-500 hover:text-primary-500 transition-colors mb-6 font-bold"
            >
                <ArrowLeft size={20} />
                Back to Recipe
            </button>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                        ...
                        {imageUrl ? (
                            ...
                            <div className="flex flex-col items-center gap-4">
                                ...
                                <div className="text-center">
                                    <p className="font-bold text-lg text-[var(--text-main)]">Update food photo</p>
                                    <p className="text-slate-500 text-sm">JPG, PNG or WEBP (Max 5MB)</p>
                                </div>
                            </div>
                        )}
                        <input type="file" onChange={handleImageChange} disabled={isUploading} className="absolute inset-0 opacity-0 cursor-pointer" />
                    </div>
                </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select value={dietType} onChange={(e) => setDietType(e.target.value)} className="input">
                        ...
                        <div className="card p-6 bg-gradient-to-br from-orange-500 to-primary-500 text-white border-none relative overflow-hidden shadow-2xl">
                            <div className="relative z-10 flex items-center justify-between">
                                <div>
                                    <p className="text-orange-100 font-bold text-sm uppercase tracking-wider mb-1">AI Nutrition Update</p>
                    ...
                    <div className="space-y-6">
                        <div>
                            <div className="flex justify-between items-center mb-4">
                                <label className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Ingredients</label>
                                <button type="button" onClick={handleAddIngredient} className="p-2 bg-primary-500 text-white rounded-xl shadow-md">
                                    <Plus size={18} />
                                </button>
                            </div>
                            <div className="space-y-3">
                                {ingredients.map((ing, idx) => (
                                    <div key={idx} className="flex gap-2">
                                        <input value={ing} onChange={(e) => handleIngredientChange(idx, e.target.value)} className="input py-2.5" placeholder={`Ingredient ${idx + 1}`} />
                                        <button type="button" onClick={() => handleRemoveIngredient(idx)} className="p-2.5 text-slate-400 hover:text-red-500 transition-colors">
                                            <Trash2 size={18} />
                                        </button>
                                    </div>
                                ))}
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-4">
                                <label className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Cooking Steps</label>
                                <button type="button" onClick={handleAddStep} className="p-2 bg-primary-500 text-white rounded-xl shadow-md">
                                    <Plus size={18} />
                                </button>
                            </div>
                            <div className="space-y-3">
                                {steps.map((step, idx) => (
                                    <div key={idx} className="flex gap-2 items-start">
                                        ...
                                        <textarea value={step} onChange={(e) => handleStepChange(idx, e.target.value)} className="input py-2.5 min-h-[60px]" placeholder={`Step ${idx + 1}`} />
                                        <button type="button" onClick={() => handleRemoveStep(idx)} className="p-2.5 text-slate-400 hover:text-red-500 pt-3">
                                            <Trash2 size={18} />
                                        </button>
                                    </div>
                                ))}
                ...
                <div className="flex justify-end gap-4 pt-10">
                    <button type="button" onClick={() => navigate(`/recipe/${id}`)} className="btn btn-secondary px-8 py-4">Discard Changes</button>
                    <button type="submit" disabled={isSubmitting || isUploading} className="btn btn-primary px-12 py-4 flex items-center gap-3 text-lg">
